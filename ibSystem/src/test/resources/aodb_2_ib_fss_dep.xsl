<xsl:stylesheet version="2.0"
	...
	<xsl:template match="/">
		...
		<IMFRoot>
			<xsl:if
				test="(/soap-env:Envelope/soap-env:Body/pl_turn/pt_pd_departure/pl_departure != '' and (/soap-env:Envelope/soap-env:Body/pl_turn/pt_pd_departure//*[@action] or $aodbMessageType = 'DATASET')) or $aodbMessageType = 'ACK'">
				<xsl:for-each select="/soap-env:Envelope/soap-env:Body">
					<SysInfo>
						...
						<ServiceType>
							<xsl:choose>
								<xsl:when test="$aodbMessageType='DATASET' or $aodbMessageType='ACK'">FSS2</xsl:when>
					...
					<xsl:if test="$aodbMessageType!='ACK'">
						<xsl:variable name="ral_2lc">
							<xsl:if test="pl_turn/pt_pd_departure/pl_departure/@action='delete'">
								<xsl:value-of
									select="normalize-space(pl_turn/pt_pd_departure/pl_departure/pd_ral_airline/ref_airline/ral_2lc/@old)" />
							</xsl:if>
							<xsl:if
								test="not(pl_turn/pt_pd_departure/pl_departure/@action) or pl_turn/pt_pd_departure/pl_departure/@action!='delete'">
								<xsl:value-of
									select="normalize-space(pl_turn/pt_pd_departure/pl_departure/pd_ral_airline/ref_airline/ral_2lc/text())" />
							</xsl:if>
						</xsl:variable>
						<xsl:for-each select="pl_turn/pt_pd_departure/pl_departure">
							<Data>
								...
								<xsl:if test="/soap-env:Envelope/soap-env:Body/pl_turn/pt_pd_departure/pl_departure/@action !='delete'">
								<FlightData>
									<General>
										...
										<FlightRoute>
											<IATARoute>
												<IATAFullRoute>
													<xsl:for-each
														select="/soap-env:Envelope/soap-env:Body/pl_turn/pt_pd_departure/pl_departure/pl_routing_list/pl_routing">
									...
									<OperationalDateTime>
										...
										<NextAirportArrivalDateTime>
											...
											<xsl:for-each select="pl_routing_list/pl_routing">
												...
												<xsl:if
													test="$nextairport=$routing_airport and $nextairport!='' and position()=1">
													<ScheduledNextAirportArrivalDateTime>
														<xsl:call-template name="xsltTemplate">
															<xsl:with-param name="inElement"
																select="prt_stastation" />
															<xsl:with-param name="needAddNil" select="true()" />
														</xsl:call-template>
													</ScheduledNextAirportArrivalDateTime>
												</xsl:if>
